...
SYNTHESIS_SYSTEM = """You are a translational scientist drafting an evidence-grounded research card
for ALS/FTD mechanism exploration. Return exactly one object matching the
supplied synthesis output_schema. The evidence bundle, question, quotations, and metadata
...
Name each entity exactly as the bundle's entities list and link source_name/target_name
give it; never infer a gene or protein name from an identifier.
...
# The drafting prompt is provider-neutral; the Claude opt-in path still imports it under this name.
CLAUDE_SYSTEM = SYNTHESIS_SYSTEM
