...
from nucleolis.analysis.adapter import CAUSAL_SIGNS
from nucleolis.analysis.grounding import aliases, catalog
...
from nucleolis.schemas.research import (
    ResearchLink, ResearchNode, ResearchPath, ResearchPlan, ResearchRequest, ResearchResponse,
)
from nucleolis.schemas.simulation import SnapshotIdentity
...
def parse_local(question: str, snap: Snapshot) -> ResearchPlan | None:
    ...
    # A bare name or common literature synonym (tau, p62) needs no model; resolve() maps the synonym.
    if q.casefold() in SYNONYMS or any(q.casefold() in {a.casefold() for a in aliases(e)} for e in snap.entities.values()):
...
_LEADING = (r"^(?:the role of|role of|the function of|function of|what is known about|known about|"
            r"lowering|raising|increasing|decreasing|inhibiting|activating|knocking out|reducing|suppressing|"
